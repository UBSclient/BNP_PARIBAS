    ...
    <div className="min-h-screen bg-muted flex flex-col items-center justify-center p-4">
      <div className="w-full max-w-md">
        ...
        <div className="bg-card rounded-2xl card-shadow p-8 animate-fade-up" style={{ animationDelay: '0.1s' }}>
          <h1 className="text-2xl font-heading font-bold text-center mb-2 text-foreground">Espace Client</h1>
          <p className="text-muted-foreground text-center mb-8">Connectez-vous à votre compte BNI</p>
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="space-y-2">
              <label htmlFor="username" className="block text-sm font-medium text-foreground">Identifiant de connexion</label>
              <div className="relative">
                ...
                <input id="username" type="text" value={username} onChange={(e) => setUsername(e.target.value)}
                  ...
                  placeholder="Entrez votre identifiant" required aria-label="Identifiant de connexion" />
              </div>
            </div>
            <div className="space-y-2">
              ...
              <div className="relative">
                ...
                <input id="password" type={showPassword ? 'text' : 'password'} value={password}
                  ...
                  placeholder="Entrez votre mot de passe" required aria-label="Mot de passe" />
            ...
            {error && (
              <div className="p-3 bg-destructive/10 border border-destructive/20 rounded-xl text-destructive text-sm text-center">{error}</div>
            )}
            <button type="submit" disabled={isLoading}
              ...
              {isLoading ? (<><div className="w-5 h-5 border-2 border-primary-foreground/30 border-t-primary-foreground rounded-full animate-spin" />Connexion en cours…</>) : 'Connexion'}
            </button>
          </form>
        </div>
